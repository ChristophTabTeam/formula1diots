...
export function CreateSeasonStep5({
  ...
  const handleCreateSeason = async () => {
    ...
    try {
      ...
      const playerData: {
        [playerId: string]: { teamId: string; slot: string };
      } = {};
      ...
      Object.keys(updatedTeams).forEach((teamId) => {
        if (!teamId || teamId === "undefined") {
          ...
          logError(
            new Error("Ungültige Team-ID gefunden"),
            user?.email?.replace("@formula1diots.de", "") || "Unbekannt",
            {
              context: "CreateSeasonStep5",
              error: "Ungültige Team-ID gefunden",
            }
          );
          return;
        }
        ...
        if (
          updatedTeams[teamId].driver1 &&
          selectedDrivers.includes(updatedTeams[teamId].driver1)
        ) {
          playerData[updatedTeams[teamId].driver1] = {
            teamId,
            slot: "driver1",
          };
        }
        if (
          updatedTeams[teamId].driver2 &&
          selectedDrivers.includes(updatedTeams[teamId].driver2)
        ) {
          playerData[updatedTeams[teamId].driver2] = {
            teamId,
            slot: "driver2",
          };
        }
      ...
      if (!seasonRules) {
        console.warn(
          "seasonRules sind null in Step 5, Standardwerte werden verwendet"
        );
      ...
      logError(
        ...
        {
          context: "CreateSeasonStep5",
          error: "Fehler beim Erstellen der Saison",
        }
      );
      setError("Fehler beim Erstellen der Saison. Bitte versuchen Sie es erneut.");
    } finally {
      setIsSaving(false);
    }
  };
