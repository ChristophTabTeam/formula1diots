...
const RaceResultsEntry: React.FC<RaceResultsEntryProps> = ({ seasonId }) => {
  const [qualifyingResults, setQualifyingResults] = useState<{ [key: string]: string }>({});
    ...
    const fetchSeasonData = async () => {
      try {
        ...
        if (seasonDoc.exists()) {
          ...
          const racesDataFiltered = racesData.filter((race) => !race.isFinished);
  
          if (racesDataFiltered.length > 0) {
            const racesDataSorted = racesDataFiltered.sort((a, b) => a.order - b.order);
  ...
  const handleSaveResults = async () => {
    ...
    setIsSaving(true);
    try {
      const raceDocRef = doc(collection(db, 'seasons', seasonId, 'races'), raceId);
      await setDoc(
        raceDocRef,
        {
          qualifyingResults: {
      ...
      if (seasonDoc.exists()) {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          if (driverId) {
            driverPoints[driverId] = (driverPoints[driverId] || 0) + pointsDistribution[i - 1];
            ...
            if (teamId) {
              teamPoints[teamId].points = (teamPoints[teamId].points || 0) + pointsDistribution[i - 1];
            }
          }
        }
  ...
  const usedQualifyingDrivers = Object.values(qualifyingResults).filter((_, index) => index % 2 === 0); // filter out lap times
  ...
  return (
    <div>
      <h1>Ergebnisse für Rennen {raceId} eintragen</h1>
      <h2>Qualifying Ergebnisse</h2>
      {[...Array(20)].map((_, index) => (
        <div key={`qualifying-${index + 1}`}>
          <label>
            Position {index + 1}:
            <select
              onChange={(e) =>
            ...
            >
              ...
              {drivers.map((driver) => (
                <option key={driver.id} value={driver.id} disabled={usedQualifyingDrivers.includes(driver.id)}>
                  {driver.name}
                </option>
              ))}
            </select>
            <input
              ...
              value={qualifyingResults[`P${index + 1}LapTime`] || ''}
              onChange={(e) =>
                handleQualifyingChange(
                  `${index + 1}`,
        ...
        <div key={`race-${index + 1}`}>
          <label>
            ...
            >
              <option value=''>Fahrer wählen</option>
              {drivers.map((driver) => (
                <option key={driver.id} value={driver.id} disabled={usedRaceDrivers.includes(driver.id)}>
                  {driver.name}
                </option>
              ))}
            </select>
